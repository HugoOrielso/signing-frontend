...
export default function LibranzaReviewDocument() {
    ...
    const [dataReviewStatus, setDataReviewStatus] = useState<DataReviewStatus | null>(null)
    useEffect(() => {
        if (!id) return;
        ...
        const getRejectedForm = async () => {
            try {
                const { data } = await api.get<{ ok: boolean; data: ReviewedLibranzaDetail[] }>(
                    `/contracts/contract/${id}/getRejectedLibranza`
                );
                ...
                setDataReviewStatus(contract.dataReviewStatus)
            } catch (error) {
                console.error(error);
            }
        };
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            const response: AxiosResponse<SuccessResponse> = await api.patch(
                `/contracts/contract/${id}/resend-libranza`,
                payload
            );


            toast.success(
                response.data.message || "Datos enviados a revisión nuevamente"
            );
            ...
            toast.error(
                axiosError.response?.data?.message || "No se pudo reenviar la información"
            );
        }
    }
    return (
        <div className='h-full flex '>
            {
                dataReviewStatus === 'PENDING' ?
                    ...
                    <div>
                        <div>
                            <div className="mb-6 rounded-2xl border border-red-200 bg-linear-to-br from-red-50 via-white to-red-50 p-5 shadow-sm">
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        <h3 className="text-base font-semibold text-red-900">
                                            Necesitamos que revises tu información
                                        </h3>

                                        <p className="mt-1 text-sm text-red-800/80">
                                            Hemos revisado tu solicitud y encontramos algunos datos que deben ser corregidos antes de continuar.
                                            <span className="font-medium"> Solo necesitas modificar la información indicada abajo.</span>
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="mt-4 text-xs text-red-700/80">
                                    👉 Corrige únicamente los datos mencionados y vuelve a enviar tu solicitud.
                                </div>
                            </div>
                        </div>
