...
export default function LibranzaUploadDocuments() {
    ...
    useEffect(() => {
        ...
        const fetchDocs = async () => {
            try {
                const { data } = await publicApiNew.get(`/users/contracts/${token}/documents`);
    ...
    const uploadDocument = async (type: DocType) => {
        ...
        try {
            ...
            const res = await publicApiNew.post(
                `/users/contracts/${token}/upload-document`,
                formData,
                {
                    headers: {
